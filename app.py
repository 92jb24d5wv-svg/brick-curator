...
if "summary" in st.session_state and "result_df" in st.session_state:
    ...
    excel_bytes = to_excel(summary, df)
...
if hist.empty:
    ...
    show_cols = [c for c in ["Analyzed At", "Set Number", "Set Name", "Buy Cost", "Part-Out Value", "ROI Multiple", "% Lots With Demand", "Score", "Recommendation"] if c in hist.columns]
