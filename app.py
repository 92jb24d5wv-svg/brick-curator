...
def top_movers_status(you_own, target_stock):
    if target_stock <= 0:
        return "⚪ No target"
    ratio = you_own / target_stock
    if ratio >= 1:
        return "🟢 Good"
    if ratio >= 0.5:
        return "🟡 Low"
...
def top_movers_target_stock(sold_qty_6mo, sold_lots_6mo, multiplier):
    base = int(max(sold_qty_6mo * multiplier, sold_lots_6mo * 3))
    return max(5, base)


def build_top_movers_from_inventory(store_df, condition, country_code, currency_code, max_lots_to_check, top_n, max_owned_qty, target_multiplier, include_parts, include_minifigs, ck, cs, tv, ts):
    """Compare likely popular BrickLink items against your store inventory.

    Version 1 starts from your active inventory, checks sold demand, then ranks the
    top movers and low-stock gaps. It is best for finding popular parts you already
    carry but may be understocked on.
    """
    if store_df.empty:
        return pd.DataFrame()
    ...
    if include_parts:
        allowed.append("PART")
    if include_minifigs:
        allowed.append("MINIFIG")

    if allowed:
        grouped = grouped[grouped["item_type"].isin(allowed)]

    grouped = grouped[grouped["store_qty"] <= int(max_owned_qty)]
    grouped = grouped.sort_values(["store_qty", "item_type", "item_no"], ascending=[True, True, True]).head(int(max_lots_to_check))

    rows = []
    progress = st.progress(0, text="Building Top Movers dashboard...")
    total = len(grouped)

    for idx, row in grouped.reset_index(drop=True).iterrows():
        progress.progress(
            min(1.0, (idx + 1) / max(total, 1)),
            text=f"Checking sold demand {idx + 1} of {total}: {row['item_no']}"
        )

        sold = get_price_guide_cached(
            row["item_type"],
            row["item_no"],
            row["color_id"],
            condition,
        ...
        sold_lots = int(sold["unit_quantity"] or 0)
        sold_qty = int(sold["total_quantity"] or 0)
        current_seller_lots = int(stock["unit_quantity"] or 0)
        you_own = int(row["store_qty"] or 0)

        if sold_lots <= 0 and sold_qty <= 0:
            continue

        target_stock = top_movers_target_stock(sold_qty, sold_lots, target_multiplier)
        suggested_restock = max(0, target_stock - you_own)
        sell_through_pressure = sold_lots / current_seller_lots if current_seller_lots > 0 else float(sold_lots)
        avg_sold_price = float(sold["qty_avg_price"] or sold["avg_price"] or 0)
        gap_value = suggested_restock * avg_sold_price

        popularity_score = 0
        popularity_score += min(45, sold_qty / 250 * 45)
        popularity_score += min(25, sold_lots / 40 * 25)
        popularity_score += min(20, sell_through_pressure / 1.5 * 20)
        popularity_score += 10 if row["item_type"] == "MINIFIG" else 0
        popularity_score = round(max(0, min(100, popularity_score)))

        gap_score = 0
        gap_score += min(50, suggested_restock / max(target_stock, 1) * 50)
        gap_score += min(30, popularity_score / 100 * 30)
        gap_score += min(20, gap_value / 50 * 20)
        gap_score = round(max(0, min(100, gap_score)))

        rows.append({
            "Popularity Score": popularity_score,
            "Gap Score": gap_score,
            "Status": top_movers_status(you_own, target_stock),
            "Item": row["item_no"],
            "Name": row["item_name"],
            "Type": row["item_type"],
            "Color": row["color_id"],
            ...
            "Sold Qty 6mo": sold_qty,
            "Sold Lots 6mo": sold_lots,
            "Current Seller Lots": current_seller_lots,
            "Sell-Through Pressure": round(sell_through_pressure, 2),
            "Avg Sold Price": round(avg_sold_price, 4),
            "Gap Value": round(gap_value, 2),
            "Drawer Remarks": row["drawer_remarks"],
        })
    ...
    df = df.sort_values(["Gap Score", "Popularity Score", "Gap Value"], ascending=[False, False, False])
    return df.head(int(top_n)).reset_index(drop=True)


def top_movers_summary_text(top_movers_df):
    if top_movers_df is None or top_movers_df.empty:
        return "No top movers found with the current filters."

    restock = top_movers_df[top_movers_df["Status"].astype(str).str.contains("Restock", na=False)]
    best = top_movers_df.iloc[0]

    return (
        f"Top mover: {best['Item']} — {best['Name']}. "
        f"You own {best['You Own']}, target stock is {best['Target Stock']}, "
        f"and suggested restock is {best['Suggested Restock']}. "
        f"{len(restock)} of the top {len(top_movers_df)} items are marked for restock."
    )
...
tool = st.radio(
    "Choose tool",
    ["Set Analyzer", "Deal Scanner", "Opportunity Finder", "Top Movers", "Brick Curator AI"],
    horizontal=True
)
...
if tool == "Brick Curator AI":
    ...
    top_movers_df = st.session_state.get("top_movers_df", pd.DataFrame())
    ...
    if st.button("Generate Brick Curator AI Briefing"):
        ...
        if not top_movers_df.empty:
            briefing["top_movers"] = top_movers_summary_text(top_movers_df)
    ...
    if "ai_briefing" in st.session_state:
        ...
        if "top_movers" in briefing:
            st.markdown("#### 📈 Top Movers Read")
            st.write(briefing["top_movers"])
...
if tool == "Top Movers":
    ...
    st.write("Compare popular/high-demand parts against your BrickLink inventory so you can see what to restock first.")

    col1, col2 = st.columns(2)
    with col1:
        tm_condition_text = st.radio("Condition", ["New", "Used"], horizontal=True, key="tm_condition")
        tm_country_code = st.text_input("Country filter", value="US", key="tm_country")
        tm_top_n = st.slider("Show top movers", 10, 100, 50, 10)
    with col2:
        tm_currency_code = st.text_input("Currency", value="USD", key="tm_currency")
        tm_max_lots = st.slider("Max inventory lots to check", 50, 500, 150, 50)
        tm_max_owned_qty = st.number_input("Only check lots where I own this many or fewer", min_value=0, max_value=1000, value=50, step=5)

    with st.expander("Top Movers settings"):
        tm_target_multiplier = st.slider(
            "Target stock multiplier",
            min_value=0.25,
            max_value=3.0,
            value=0.75,
            step=0.25,
            help="Higher = more aggressive restock targets based on 6-month sold quantity."
        )
        tm_include_parts = st.checkbox("Include parts", value=True)
        tm_include_minifigs = st.checkbox("Include minifigs", value=True)
        st.caption("Version 1 checks your existing low-stock inventory, then ranks the most popular understocked items.")

    if st.button("Build Top Movers Dashboard"):
        if not all([ck, cs, tv, ts]):
            st.error("Add your BrickLink API keys first.")
        else:
            tm_condition = "N" if tm_condition_text == "New" else "U"

            try:
                with st.spinner("Downloading your active BrickLink inventory..."):
                    store_df = get_store_inventory_all_cached(tm_condition, ck, cs, tv, ts)

                if store_df.empty:
                    st.warning("No active inventory found for this condition.")
                else:
                    top_movers_df = build_top_movers_from_inventory(
                        store_df,
                        tm_condition,
                        tm_country_code.strip().upper(),
                        tm_currency_code.strip().upper(),
                        tm_max_lots,
                        tm_top_n,
                        tm_max_owned_qty,
                        tm_target_multiplier,
                        tm_include_parts,
                        tm_include_minifigs,
                        ck,
                        cs,
                    ...
                    st.session_state["top_movers_df"] = top_movers_df

            except Exception as e:
                st.error(str(e))

    if "top_movers_df" in st.session_state:
        top_movers_df = st.session_state["top_movers_df"]

        st.markdown("### Top 50 Popular Parts vs Your Inventory")
        if top_movers_df.empty:
            st.info("No top movers found. Try increasing max inventory lots checked or raising the owned-quantity limit.")
        else:
            restock_count = int(top_movers_df["Status"].astype(str).str.contains("Restock", na=False).sum())
            low_count = int(top_movers_df["Status"].astype(str).str.contains("Low", na=False).sum())
            gap_value = float(pd.to_numeric(top_movers_df["Gap Value"], errors="coerce").sum())

            c1, c2 = st.columns(2)
            c1.metric("Restock Flags", restock_count)
            c2.metric("Total Gap Value", f"${gap_value:,.2f}")

            c3, c4 = st.columns(2)
            c3.metric("Low Stock Flags", low_count)
            c4.metric("Items Shown", len(top_movers_df))

            st.info(top_movers_summary_text(top_movers_df))

            display_cols = [
                "Popularity Score", "Gap Score", "Status", "Item", "Name", "Type", "Color",
                "You Own", "Target Stock", "Suggested Restock", "Sold Qty 6mo",
                "Sold Lots 6mo", "Current Seller Lots", "Sell-Through Pressure",
                "Avg Sold Price", "Gap Value", "Drawer Remarks"
            ]

            st.dataframe(top_movers_df[display_cols], use_container_width=True, hide_index=True)

            st.download_button(
                "Download Top Movers CSV",
                top_movers_df.to_csv(index=False),
                file_name="brick_curator_top_movers.csv",
                mime="text/csv",
            )
